...
regions=["nAk8G0"]
channels=["aJets","fakeAJetsIso", "fakeAJetsSiSi","fakeAJetsIsoSiSi","fakeAJetsOthers"];

variables=["GammaPt","GammaEta", "nPh"]
variablesName=["p_{T}(#gamma)","#eta(#gamma)","#Phi(#gamma)","p_{T}(leading jet (AK4))","#eta(leading jet (AK4))","Number of photons","phoChargedIso","#sigma_{i#eta i#eta}", "HoE"]
...
for numyear, nameyear in enumerate(year):
    for numreg, namereg in enumerate(regions):
        for numvar, namevar in enumerate(variables):
#data driven fake rate
            hB = Hists[numyear][0][0][channels.index("fakeAJetsIso")][numreg][numvar]
            hD = Hists[numyear][0][0][channels.index("fakeAJetsSiSi")][numreg][numvar]
            hC = Hists[numyear][0][0][channels.index("fakeAJetsIsoSiSi")][numreg][numvar]
            hO = Hists[numyear][0][0][channels.index("fakeAJetsOthers")][numreg][numvar]
            hOContamination = Hists[numyear][0][1][channels.index("fakeAJetsOthers")][numreg][numvar]
            hO.Add(hOContamination,-1)
            hA = hB.Clone()
            hA.Multiply(hD)
            hA.Divide(hC)
            hA2 = hA.Clone()
            ...
            hA.Divide(hA2)
#MC fake rate
            hBMC = Hists[numyear][2][3][channels.index("fakeAJetsIso")][numreg][numvar]
            hDMC = Hists[numyear][2][3][channels.index("fakeAJetsSiSi")][numreg][numvar]
            hCMC = Hists[numyear][2][3][channels.index("fakeAJetsIsoSiSi")][numreg][numvar]
            hOMC = Hists[numyear][2][3][channels.index("fakeAJetsOthers")][numreg][numvar]
            hAMC = Hists[numyear][2][3][channels.index("aJets")][numreg][numvar]
            hA2MC = hAMC.Clone()
            hA2MC.Add(hDMC)
            hA2MC.Add(hCMC)
            hA2MC.Add(hBMC)
            ...
            pad1=ROOT.TPad(pad_name, pad_name, 0.05, 0.05, 1, 0.99 , 0)
            pad1.Draw("")
            hA.SetTitle(nameyear)
            hA.SetName(nameyear+"_FakeRateVsEta")
            hA.GetYaxis().SetTitle('jet #rightarrow #gamma fake rate')
            hA.GetXaxis().SetTitle(namevar)
            hA.GetXaxis().SetTitleSize(0.05)
            hA.GetYaxis().SetTitleSize(0.05)
            hA.GetYaxis().SetTitleOffset(0.7)
            hA.SetLineColor(2)
            hA.SetLineWidth(2)
            hA.GetYaxis().SetRangeUser(0,0.05)
            hA.Draw("e")
            if namevar=='GammaEta':
                HH.append(hA)
            hAMC.SetLineWidth(2)
            ...
            legend.SetTextFont(42)
            legend.SetTextSize(0.04)
            legend.AddEntry(hA ,'Data-driven','l')
            legend.AddEntry(hAMC ,'MC','l')
            legend.Draw("Same")
            canvas.Print( "AJetFR"+nameyear + '_' + namereg + '_' + namevar+".png")

hfile = ROOT.TFile( 'JetFakeRate.root', 'RECREATE', 'jet-->photon fake rate histogram' )
for numyear, nameyear in enumerate(year):
    HH[numyear].Write()
hfile.Write()
hfile.Close()
